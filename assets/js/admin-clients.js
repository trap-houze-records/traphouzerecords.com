window.ClientAdminModule = (() => {
  ...
  function mount(root, options) {
    ...
    function recordForm(type, item) {
      ...
      const recording = track && item.category === 'recording';
      ...
      const recordingFileRows = versions.map(version => `<div class="manager-track-version"><div><b>${escapeHtml(version.originalName)}</b></div><div><button type="button" data-admin-version-play="${escapeHtml(version.id)}" data-version-label="Gravação" data-track-id="${escapeHtml(item.id)}">Ouvir</button><button type="button" class="danger" data-admin-version-delete="${escapeHtml(version.id)}" data-track-id="${escapeHtml(item.id)}">Apagar</button></div></div>`).join('');
      const automaticFirstLabel = item.requestedService === 'master' ? 'Mix' : 'Gravação';
      ...
      const trackConfiguration = track && !recording ? `<div class="manager-record-grid"><label class="admin-field"><span>Área</span><select data-field="category"><option value="mix-master" selected>Mix & Master</option><option value="recording">Gravações</option></select></label><label class="admin-field"><span>Fase</span><select data-field="stage">${[['start', 'Gravação'], ['mix', 'Mix'], ['master', 'Master']].map(([value, text]) => `<option value="${value}" ${item.stage === value ? 'selected' : ''}>${text}</option>`).join('')}</select></label></div><div class="manager-record-grid"><label class="admin-field"><span>Serviço pedido</span><select data-field="requestedService"><option value="">Sem pedido</option>${mixServices.map(service => `<option value="${escapeHtml(service.id)}" ${item.requestedService === service.id ? 'selected' : ''}>${escapeHtml(service.title)}</option>`).join('')}</select></label><label class="admin-field"><span>Gravação de origem</span><select data-field="sourceTrackId"><option value="">Nenhuma</option>${recordingOptions}</select></label></div>` : track ? '' : `<label class="admin-field"><span>Data e hora</span><input type="datetime-local" data-field="startsAt" value="${escapeHtml((item.startsAt || '').replace(' ', 'T').slice(0, 16))}"></label>`;
      const billingFields = recording ? '' : `<div class="manager-record-grid"><label class="admin-field"><span>Valor (€)</span><input type="number" step="0.01" min="0" data-field="amount" value="${numericAmount(item.amountCents)}"></label><label class="admin-field"><span>Recebido (€)</span><input type="number" step="0.01" min="0" data-field="paidAmount" value="${payment.paid.toFixed(2)}"></label></div><label class="admin-field"><span>Link de pagamento</span><input type="url" data-field="paymentUrl" value="${escapeHtml(item.paymentUrl || '')}" placeholder="https://"></label>`;
      const recordingWorkspace = `<div class="manager-track-workspace"><strong>Ficheiro da gravação</strong><div class="manager-track-version-list">${recordingFileRows || '<small>Nenhum ficheiro carregado.</small>'}</div>${versions.length ? `<audio controls preload="none" data-admin-track-audio="${escapeHtml(item.id)}"></audio><small data-admin-track-time>Escolhe o ficheiro para ouvir.</small>` : ''}<form data-admin-track-upload="${escapeHtml(item.id)}"><input name="file" type="file" accept="audio/mpeg,audio/wav,audio/x-wav,audio/flac,audio/mp4,audio/aac,audio/ogg" required><input name="label" type="hidden" value="Gravação"><button type="submit">Enviar ficheiro</button></form></div>`;
      const mixWorkspace = `<div class="manager-track-workspace"><strong>Versões no Trap Houze Player (${versions.length})</strong><div class="manager-track-version-list">${versionRows || '<small>Sem versões carregadas.</small>'}</div>${versions.length ? `<audio controls preload="none" data-admin-track-audio="${escapeHtml(item.id)}"></audio><small data-admin-track-time>Escolhe uma versão para ouvir.</small>` : ''}<form data-admin-track-upload="${escapeHtml(item.id)}"><input name="file" type="file" accept="audio/mpeg,audio/wav,audio/x-wav,audio/flac,audio/mp4,audio/aac,audio/ogg" required>${versions.length ? '<input name="label" maxlength="120" placeholder="Nome da versão">' : `<input name="label" type="hidden" value="${automaticFirstLabel}"><small>Primeira versão: ${automaticFirstLabel}</small>`}<button type="submit">Enviar versão</button></form></div><div class="manager-track-workspace manager-track-comments"><button type="button" class="manager-comments-toggle" data-toggle-admin-comments="${escapeHtml(item.id)}" aria-expanded="${commentsOpen ? 'true' : 'false'}"><strong>Comentários</strong><span>${comments.length}</span></button><div data-admin-comments-panel ${commentsOpen ? '' : 'hidden'}>${commentRows || '<small>Sem comentários.</small>'}${versions.length ? `<form data-admin-track-comment="${escapeHtml(item.id)}"><select name="versionId" required>${versionOptions}</select><input name="body" maxlength="2000" placeholder="Comentar o ponto atual" required><button type="submit">Comentar</button><small>O comentário fica associado à versão escolhida e ao tempo atual do player.</small></form>` : '<small>Adiciona uma versão antes de comentar.</small>'}</div></div>`;
      return `<article class="manager-record" data-item="${type}" data-id="${item.id}" ${recording ? 'data-track-category="recording"' : ''}>
        <div class="manager-record-top"><strong>${track ? (recording ? 'Gravação' : 'Mix & Master') : 'Reserva'}</strong><button type="button" data-delete="${type}" data-id="${item.id}" aria-label="Remover">×</button></div>
        <label class="admin-field"><span>${label}</span><input data-field="${name}" value="${escapeHtml(item[name])}"></label>
        ${trackConfiguration}
        ${billingFields}
        ${track ? (recording ? recordingWorkspace : mixWorkspace) : ''}
       ${recording ? '' : `<p class="admin-hint">${payment.label}</p>`}
       <button type="button" class="manager-copy" data-save-record>Guardar ${track ? (recording ? 'gravação' : 'música') : 'reserva'}</button>
      </article>`;
    }
    ...
    function render() {
      ...
      <section class="manager-content"><div class="manager-content-heading"><div><p class="eyebrow">Cliente selecionado</p><h2>${escapeHtml(client.name)}</h2></div></div><div class="manager-grid">
        ...
        <article class="client-card"><div class="manager-title-row"><h3>Mix & Master</h3><button class="client-link" type="button" data-client-add="tracks" data-track-category="mix-master">+ Música</button></div><div class="manager-records">${portal.tracks.filter(item => item.category !== 'recording').map(item => recordForm('tracks', item)).join('') || '<p class="admin-hint">Sem músicas em Mix & Master.</p>'}</div></article>
        <article class="client-card"><div class="manager-title-row"><h3>Gravações</h3><button class="client-link" type="button" data-client-add="tracks" data-track-category="recording">+ Gravação</button></div><div class="manager-records">${portal.tracks.filter(item => item.category === 'recording').map(item => recordForm('tracks', item)).join('') || '<p class="admin-hint">Sem gravações registadas.</p>'}</div></article>
    ...
    function payload(card) {
      ...
      if (track && card.dataset.trackCategory === 'recording') return { title: field('title').value.trim(), stage: 'start', category: 'recording', requestedService: '', sourceTrackId: '', amount: 0, paidAmount: 0, paymentUrl: '', samplyUrl: '' };
    ...
    root.addEventListener('click', event => {
      ...
      if (add) {
        ...
        const category = add.dataset.trackCategory || 'mix-master';
        const body = type === 'tracks' ? { title: category === 'recording' ? 'Nova gravação' : 'Nova música', stage: 'start', category, amount: 0, paymentStatus: 'pending' } : { service: 'Nova reserva', startsAt: '', amount: 0, paymentStatus: 'pending' };
        return api(`/client/admin/${type}/${portal.client.id}`, { method: 'POST', body: JSON.stringify(body) }).then(() => {
          if (type === 'tracks') options.onNotice?.(category === 'recording' ? 'Gravação criada. Adiciona agora o ficheiro de áudio.' : 'Música criada.', 'success');
          return refresh();
        }).catch(options.onError);
      }
