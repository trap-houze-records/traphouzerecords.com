...
function renderArtists() { return `<section>${listCards('artists', (item, index) => card(item.name || `Artista ${index + 1}`, `${field('Nome', `artists.${index}.name`, item.name)}${field('Género / função', `artists.${index}.genre`, item.genre)}${field('Imagem (URL)', `artists.${index}.image`, item.image)}${field('Biografia', `artists.${index}.bio`, item.bio || '', 'textarea')}${field('Ligações (Nome | URL por linha)', `artists.${index}.links`, (item.links || []).map(link => `${link.label || ''} | ${link.url || ''}`).join('\\n'), 'textarea')}${field('Catálogo Trap Houze (Título | Tipo | URL por linha; use Playlist Spotify para incorporar preview)', `artists.${index}.catalog`, (item.catalog || []).map(entry => `${entry.title || ''} | ${entry.type || ''} | ${entry.url || ''}`).join('\\n'), 'textarea')}`, `<button class="admin-icon" data-remove="artists" data-index="${index}" aria-label="Remover">×</button>`), 'Adicionar artista')}</section>`; }
...
function addItem(collection) { const models = { hero: { title: 'Novo destaque', subtitle: '', image: '' }, services: { title: 'Novo serviço', description: '', icon: '✦', action: 'booking', visible: true }, equipment: { id: 'novo-item', title: 'Novo equipamento', description: '', images: [] }, artists: { name: 'Novo artista', genre: '', image: '', bio: '', links: [], catalog: [] }, reviews: { name: 'Nome', text: '' } }; draft[collection].push(models[collection]); renderTab(); }
function removeItem(collection, index) { draft[collection].splice(index, 1); renderTab(); }
function parseSpecial(path, value) { if (path.endsWith('.images')) return value.split('\n').map(item => item.trim()).filter(Boolean); if (path === 'about.paragraphs') return value.split('\n').map(item => item.trim()).filter(Boolean); if (path.endsWith('.links')) return value.split('\n').map(line => line.split('|').map(part => part.trim())).filter(parts => parts[0] || parts[1]).map(([label, url]) => ({ label, url })); if (path.endsWith('.catalog')) return value.split('\n').map(line => line.split('|').map(part => part.trim())).filter(parts => parts[0]).map(([title, type, url]) => ({ title, type, url })); return value; }
