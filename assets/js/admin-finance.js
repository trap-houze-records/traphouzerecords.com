  ...
  function mount(root, options) {
    ...
    function render(data) {
      ...
      root.innerHTML = `<section class="finance">
        ...
        <section class="finance-pricing finance-pricing-compact"><div class="finance-pricing-head"><div><p class="eyebrow">Configuração</p><h3>Preços das reservas</h3><p>Aplicados automaticamente a novas marcações.</p></div><button type="button" class="finance-save" data-save-booking-prices>Guardar preços</button></div><div class="finance-pricing-list"><div class="finance-price-columns"><span>Serviço</span><span>€/hora</span><span>Estado</span></div>${bookingServices.map((service, index) => `<div class="finance-price-row"><label><span class="sr-only">Serviço</span><input aria-label="Serviço" data-booking-price="title" data-index="${index}" value="${escapeHtml(service.title)}"></label><label><span class="sr-only">Preço por hora</span><input aria-label="Preço por hora" type="number" min="0" step="0.01" data-booking-price="pricePerHour" data-index="${index}" value="${Number(service.pricePerHour || 0)}"></label><label class="finance-price-availability"><input type="checkbox" data-booking-price="active" data-index="${index}" ${service.active !== false ? 'checked' : ''}><span>Disponível</span></label></div>`).join('')}</div></section>
        ...
        <div class="finance-grid"><section class="finance-panel"><div class="finance-panel-head"><div><p class="eyebrow">A receber</p><h3>Pagamentos pendentes</h3></div><span>${pending.length}</span></div><div class="finance-list">${pending.length ? pending.map(item => `<article class="finance-item"><div><strong>${escapeHtml(item.description)}</strong><span>${escapeHtml(item.clientName)} · ${escapeHtml(recordLabel(item))}</span></div><div><b>${money(item.amountCents)}</b>${item.paymentUrl ? `<a href="${escapeHtml(item.paymentUrl)}" target="_blank" rel="noopener">Pagamento →</a>` : '<small>Sem link de pagamento</small>'}</div></article>`).join('') : '<p class="admin-hint">Não existem pagamentos pendentes.</p>'}</div></section>
        ...
        <section class="finance-panel finance-history"><div class="finance-panel-head"><div><p class="eyebrow">Registos</p><h3>Histórico financeiro</h3></div></div><div class="finance-table"><div class="finance-table-head"><span>Descrição</span><span>Cliente</span><span>Data</span><span>Estado</span><span>Valor</span></div>${data.items.map(item => `<article><span><strong>${escapeHtml(item.description)}</strong><small>${escapeHtml(recordLabel(item))}</small></span><span>${escapeHtml(item.clientName)}</span><span>${date(item.createdAt)}</span><span class="finance-status ${item.paymentStatus}">${item.paymentStatus === 'paid' ? 'Pago' : 'Pendente'}</span><strong>${money(item.amountCents)}</strong></article>`).join('') || '<p class="admin-hint">Ainda não existem registos financeiros.</p>'}</div></section></section>`;
    }
