  ...
  function mount(root, options) {
    ...
    function render(data) {
      ...
      root.innerHTML = `<section class="finance"><header class="studio-editor-heading"><div><p class="eyebrow">Visão financeira</p><h2>Resumo financeiro</h2><p>Acompanha valores recebidos, pendentes e respetivos clientes. A edição dos pagamentos mantém-se na área Clientes.</p></div><div class="studio-editor-state"><span>Em tempo real</span><small>Dados das músicas e reservas registadas.</small></div></header>
        <section class="finance-panel finance-pricing"><div class="finance-panel-head"><div><p class="eyebrow">Configuração</p><h3>Preços das reservas</h3></div><span>€/h</span></div><p class="admin-hint">Estes valores entram automaticamente em novas marcações feitas no Admin ou na Agenda.</p><div class="finance-pricing-list">${bookingServices.map((service, index) => `<div class="admin-row"><label class="admin-field"><span>Tipo de reserva</span><input data-booking-price="title" data-index="${index}" value="${escapeHtml(service.title)}"></label><label class="admin-field"><span>Preço por hora (€)</span><input type="number" min="0" step="0.01" data-booking-price="pricePerHour" data-index="${index}" value="${Number(service.pricePerHour || 0)}"></label><label class="admin-check"><input type="checkbox" data-booking-price="active" data-index="${index}" ${service.active !== false ? 'checked' : ''}><span>Disponível</span></label></div>`).join('')}</div><button type="button" class="manager-copy" data-save-booking-prices>Guardar preços</button></section>
