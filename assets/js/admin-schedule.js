  ...
  function mount(root, options) {
    ...
    root.addEventListener('click', event => {
      ...
      if (event.target.closest('[data-save-schedule-settings]')) {
        root.querySelectorAll('[data-service-rule]').forEach(card => { const id = card.dataset.serviceRule; scheduleConfig.serviceRules[id] = Object.fromEntries([...card.querySelectorAll('[data-rule]')].map(input => [input.dataset.rule, input.dataset.rule === 'minNoticeHours' ? Number(input.value || 0) : input.value])); });
