  ...
  function mount(root, options) {
    ...
    const validateScheduleConfig = config => {
      for (const service of bookingServices.filter(item => item.active !== false)) {
        ...
        if (startsAt === null || endsAt === null || startsAt >= endsAt) throw new Error(`Defina um período válido para “${service.title}”.`);
        if (rule?.lunchEnabled) {
          const lunchStartsAt = ruleMinutes(rule.lunchStartsAt); const lunchEndsAt = ruleMinutes(rule.lunchEndsAt);
          if (lunchStartsAt === null || lunchEndsAt === null || lunchStartsAt >= lunchEndsAt || lunchStartsAt < startsAt || lunchEndsAt > endsAt) throw new Error(`A pausa de almoço de “${service.title}” tem de estar dentro do período disponível.`);
        }
      }
    ...
    function defaultAppointment() {
      ...
      return { id: '', clientId: '', clientName: '', clientEmail: '', service: bookingServices[0].title, serviceId: bookingServices[0].id, startsAt: localInput(start), endsAt: localInput(end), amount: bookingServices[0].pricePerHour, paymentStatus: 'pending', paymentUrl: '', status: 'confirmed', notes: '' };
    }
    ...
    root.addEventListener('click', event => {
      ...
      if (appointment) { selected = appointments.find(item => item.id === appointment.dataset.appointment) || null; draft = selected ? { ...selected, serviceId: serviceFor(selected).id, amount: Number(selected.amountCents || 0) / 100, paymentStatus: selected.paymentStatus || 'pending', paymentUrl: selected.paymentUrl || '' } : null; return render(); }
    ...
    root.addEventListener('change', event => {
      ...
      if (!['clientId', 'serviceId', 'startsAt', 'endsAt'].includes(changed)) return;
      draft = readForm();
